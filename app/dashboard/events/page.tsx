...
export default function EventsDashboard() {
  ...
  const handleAdd = async () => {
    if (!newEvent.title || !newEvent.date || !newEvent.location || !newFile || !newEvent.description)
      return addToast({ title: "Missing inputs", description: "Please fill out all fields", color: "warning" });

    const imageUrl = await uploadFile(newFile);
    if (!imageUrl) return;

    try {
      const res = await fetch("/api/events", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...newEvent, image: imageUrl, date: new Date(newEvent.date) }) });
      const saved = await res.json();
      setEvents(prev => [...prev, saved]);
      setNewEvent({ title: "", date: "", location: "", image: "", description: "" });
      setNewFile(null);
      onClose();
      addToast({ title: "Added", description: "Event added successfully", color: "success" });
    } catch {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await fetch(`/api/events/${editEvent._id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...editEvent, image: imageUrl, date: new Date(editEvent.date) }) });
      setEvents(prev => prev.map(e => e._id === editEvent._id ? { ...editEvent, image: imageUrl } : e));
      setEditFile(null);
      onEditClose();
      addToast({ title: "Updated", description: "Event updated successfully", color: "success" });
    } catch {
  ...
  return (
    <div className="flex flex-col gap-6 px-8 py-4">
      ...
      {/* Edit Modal */}
      {editEvent && <Modal backdrop="blur" isOpen={isEditOpen} onClose={onEditClose}>
        <ModalContent>{() => <>
          ...
          <ModalBody>
            ...
            <Textarea placeholder="Description" value={editEvent.description} onChange={e => setEditEvent({ ...editEvent, description: e.target.value })} />
          </ModalBody>
          <ModalFooter>
      ...
      {loading ? (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {[...Array(3)].map((_, i) => <Card key={i} className="p-6"><Skeleton className="h-48 mb-4" /><Skeleton className="h-4 w-1/2 mb-2" /><Skeleton className="h-3 w-5/6 mb-1" /><Skeleton className="h-3 w-4/6 mb-3" /><div className="flex gap-2 mb-2"><Skeleton className="h-3 w-1/3" /><Skeleton className="h-3 w-1/4" /></div><div className="flex gap-2 pt-2"><Skeleton className="h-8 w-1/2 rounded-md" /><Skeleton className="h-8 w-1/2 rounded-md" /></div></Card>)}
        </div>
      ) : (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {events.map(event => (
            <Card key={event._id} className="relative overflow-hidden group flex flex-col h-full before:absolute before:inset-0">
              ...
              <CardBody className="p-4 space-y-1 flex flex-col flex-grow min-h-[180px]">
                <div className="flex justify-between items-start mb-2"><h3 className="font-semibold text-sm">{event.title}</h3></div>
                <p className="text-xs text-muted-foreground pb-2 flex-grow">{event.description}</p>
                <div className="flex items-center gap-2 mt-auto"><CalendarIcon className="w-5 h-5 text-muted-foreground" /> <p className="text-xs text-muted-foreground">{new Date(event.date).toLocaleDateString()}</p></div>
                <div className="flex items-center gap-2"><GlobeIcon className="w-5 h-5 text-muted-foreground" /> <p className="text-xs text-muted-foreground">{event.location}</p></div>
